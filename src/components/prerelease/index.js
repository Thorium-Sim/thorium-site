...
export default class Prerelease extends Component {
  ...
  access = () => {
    ...
    request(graphQLURL, query, {email})
    .then(data => {
  ...
  render() {
    return <div>
      ...
      <h1>Please enter your email address to access prerelease versions of Thorium.</h1>
      <input type="text" onChange={(evt) => this.setState({email: evt.target.value})} value={this.state.email} />
